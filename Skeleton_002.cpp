...
void Render()
{
	...
	if (loadnewfile)
	{
		...
		seed_dxyz = readCSV(fname_list[file_id].c_str(), 0);

		cout << "Loading " << fname_list[pair(file_id)] << "...\n";
		pred_dxyz = readCSV(fname_list[pair(file_id)].c_str(), 0);
	...
	ss << frame_id << "/" << frames_N << "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t file_id: " << file_id;
